  ...
  switch(msj.type) {
      case "arrival":
        ...
        if(msj.id != ownID) {
          ...
          console.log(msj.id)
          client.publish(channel, { type: "Existence notification", id: ID, x: ships[ID].x, y: ships[ID].y, angle: ships[ID].angle, sprite: SPRITEPATH, health: ships[ID].health, points: ships[ID].points })
        }
        break;
